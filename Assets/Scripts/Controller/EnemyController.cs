...
public class EnemyController : BaseController
{
    ...
    {
        GameObject hudText = Instantiate(hudDamageText); // ?ì„±???ìŠ¤???¤ë¸Œ?íŠ¸
        hudText.transform.position = transform.position + Vector3.up*1.5f; // ?œì‹œ???„ì¹˜
        hudText.GetComponent<UI_DamageText>()._damage = damage; // ?°ë?ì§€ ?„ë‹¬
    }
